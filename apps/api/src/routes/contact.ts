...
import { contactRequests, users, donors, notifications } from '@nirbenu/db';
...
contactRoutes.post('/', requireAuth, zValidator('json', contactRequestSchema), async (c) => {
  ...
  const existing = await db
    ...
    .where(and(eq(contactRequests.requesterId, userId), eq(contactRequests.donorId, donorId), eq(contactRequests.status, 'pending')))
    .limit(1);

  if (existing.length) return c.json({ error: 'Contact request already sent' }, 409);

  await db.insert(contactRequests).values({
    id, requesterId: userId, donorId, message: message ?? null,
    status: 'pending', numberVisible: false, createdAt: now, updatedAt: now,
  });

  await db.insert(notifications).values({
    id: uuid(), userId: donorId, type: 'contact_received',
    title: 'New Contact Request', message: 'Someone wants to contact you for blood donation.',
...
contactRoutes.patch('/:id/accept', requireAuth, async (c) => {
  ...
  await db.update(contactRequests).set({ status: 'accepted', numberVisible: true, updatedAt: now }).where(eq(contactRequests.id, id));

  await db.insert(notifications).values({
    id: uuid(), userId: req[0].requesterId, type: 'contact_accepted',
...
contactRoutes.patch('/:id/reject', requireAuth, async (c) => {
  ...
  await db.update(contactRequests).set({ status: 'rejected', updatedAt: now }).where(eq(contactRequests.id, id));
