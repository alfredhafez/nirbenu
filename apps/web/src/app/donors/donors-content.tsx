...
import {
  Search, MapPin, Filter, X, Shield, Droplets,
          ...
          <div className="flex flex-col lg:flex-row gap-6">
              ...
              <aside className="lg:w-64 shrink-0">
                <div className="glass-card rounded-2xl p-5 space-y-4 sticky top-20">
                  <div className="flex items-center justify-between">
                    ...
                    <button onClick={() => { setSelBlood(''); setSelDistrict(''); setSelAvailability(''); updateParams({ bloodGroup: '', district: '', availability: '' }); }} className="text-xs text-primary hover:underline">Clear All</button>
                  </div>
                  <div>
                    <label className="text-xs font-medium text-slate-500 mb-2 block">Blood Group</label>
                    <div className="flex flex-wrap gap-1.5">
                      {BLOOD_GROUPS.map((bg) => (
                        <button key={bg} onClick={() => { setSelBlood(selBlood === bg ? '' : bg); updateParams({ bloodGroup: selBlood === bg ? '' : bg }); }} className={cn('px-3 py-1.5 rounded-lg text-xs font-medium border transition-all', selBlood === bg ? 'bg-primary text-white border-primary' : 'bg-white text-slate-600 border-slate-200 hover:border-primary-300')}>{bg}</button>
                      ))}
                    </div>
                  </div>
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {DISTRICTS.slice(0, 8).map((d) => (
                        <button key={d} onClick={() => { setSelDistrict(selDistrict === d ? '' : d); updateParams({ district: selDistrict === d ? '' : d }); }} className={cn('px-3 py-1.5 rounded-lg text-xs font-medium border transition-all', selDistrict === d ? 'bg-primary text-white border-primary' : 'bg-white text-slate-600 border-slate-200 hover:border-primary-300')}>{d}</button>
                      ))}
                    </div>
                  </div>
                  <div>
                    ...
                    <div className="space-y-1">
                      {AVAILABILITY_OPTIONS.map((a) => (
                        <button key={a} onClick={() => { setSelAvailability(selAvailability === a ? '' : a); updateParams({ availability: selAvailability === a ? '' : a }); }} className={cn('w-full text-left px-3 py-1.5 rounded-lg text-xs font-medium transition-all border', selAvailability === a ? 'bg-primary text-white border-primary' : 'bg-white text-slate-600 border-slate-200 hover:border-primary-300')}>{getAvailabilityLabel(a)}</button>
                      ))}
                    </div>
                  </div>
                </div>
            ...
            <div className="flex-1">
              <div className="relative mb-6">
                ...
                <Input className="pl-10" placeholder="Search by name or area..." value={search} onChange={(e) => setSearch(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter') updateParams({ search }); }} />
              </div>
                  ...
                  <Link key={donor.id} href={`/donors/${donor.id}`}>
                    <Card hover className="h-full">
                      <div className="flex items-center gap-3 mb-3">
                        <div className="flex h-11 w-11 items-center justify-center rounded-full bg-primary-100 text-primary-700 font-bold">{donor.user.name.charAt(0)}</div>
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-1 text-xs text-slate-500"><MapPin className="h-3 w-3" /> {donor.area}, {donor.district}</div>
                        </div>
                      </div>
                      <div className="flex items-center gap-2 mb-3 flex-wrap">
                        <span className="px-2.5 py-0.5 rounded-md bg-red-50 text-red-600 text-xs font-bold border border-red-200">{donor.bloodGroup}</span>
                        <span className={`px-2 py-0.5 rounded-full text-[10px] font-medium ${getRankingColor(donor.ranking)}`}>{donor.ranking.replace('_', ' ')}</span>
                      </div>
                      <div className="flex items-center justify-between text-xs">
                        <div className="flex items-center gap-1 text-slate-500"><Droplets className="h-3 w-3 text-primary" /> {donor.donationCount} donations</div>
                        <span className={cn('px-2 py-0.5 rounded-full text-[10px] font-medium', getAvailabilityColor(donor.availability))}>{getAvailabilityLabel(donor.availability)}</span>
                      </div>
                    </Card>
                  </Link>
                ))}
              ...
              {totalPages > 1 && (
                <div className="flex items-center justify-center gap-2 mt-8">
                  ...
                  {Array.from({ length: totalPages }, (_, i) => (
                    <Button key={i} variant={page === i + 1 ? 'default' : 'outline'} size="sm" className="w-9" onClick={() => updateParams({ page: String(i + 1) })}>{i + 1}</Button>
                  ))}
