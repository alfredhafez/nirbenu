...
import {
  LayoutDashboard, ClipboardList, MessageSquare, Clock,
  Settings, LogOut, Bell, Menu, X, Droplets, Heart,
} from 'lucide-react';
import { Button } from '@/components/ui/button';
import { cn } from '@/lib/utils';

const sidebarLinks = [
  { href: '/donor/dashboard', icon: LayoutDashboard, label: 'Overview' },
  { href: '/donor/requests', icon: ClipboardList, label: 'Requests' },
  { href: '/donor/messages', icon: MessageSquare, label: 'Messages' },
  { href: '/donor/history', icon: Clock, label: 'History' },
  { href: '/donor/recovery', icon: Heart, label: 'Recovery' },
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="h-16 border-b border-slate-200 bg-white flex items-center justify-between px-6">
          ...
          <div className="flex items-center gap-3 ml-auto">
            ...
            <div className="flex h-9 w-9 items-center justify-center rounded-full bg-red-100 text-red-600 font-semibold text-sm">
              D
            </div>
          </div>
        </header>
